...
describe('HasteMap', () => {
  beforeEach(() => {
    ...
    const entryPoint = require('../index.ts');

    HasteMap = entryPoint.HasteMap
    H = HasteMap.H;
  ...
  it('creates different cache file paths for different roots', async () => {
    jest.resetModules();
    const entryPoint = require('../index');
    const hasteMap1 = await entryPoint.HasteMap.create({
      ...defaultConfig,
      rootDir: '/root1',
    });
    const hasteMap2 = await entryPoint.HasteMap.create({
      ...defaultConfig,
      rootDir: '/root2',
    });
